...
public class IntegrityResource {
    ...
    private String resolveRemoteIp(@Context HttpServletRequest request) {
        String remoteIP;
    ...
    public Response generateIntegrityData(
        ...
        try {
            ...
            if (QuartzUtils.isJobRunning(
                    ...
                    IntegrityDataGenerationJob.JOB_GROUP_NAME)) {
            ...
            Logger.error(IntegrityResource.class, String.format("Error caused by remote call of: %s", remoteIp));
    ...
    public Response getIntegrityData(@Context final HttpServletRequest request,
        ...
        try {
            ...
            if (controlOpt.get().getProcessStatus() == ProcessStatus.PROCESSING) {
                return Response.status(HttpStatus.SC_PROCESSING).build();
            } else if (controlOpt.get().getProcessStatus() == ProcessStatus.FINISHED &&
                ...
                if (jobStatusOpt.isPresent() &&
                        (jobStatusOpt.get().getProcessStatus() == ProcessStatus.CANCELLED ||
                                jobStatusOpt.get().getProcessStatus() == ProcessStatus.ERROR)) {
                    return Response
                            .status(jobStatusOpt.get().getProcessStatus() == ProcessStatus.CANCELLED
                                    ? HttpStatus.SC_RESET_CONTENT
                                    : HttpStatus.SC_INTERNAL_SERVER_ERROR)
                            .entity(jobStatusOpt.get().getErrorMessage())
                            .build();
                }
            }
        } catch (Exception e) {
    ...
    private Optional<ScheduledTask> findScheduled(final String endpointId) {
        try {
            return QuartzUtils
                    .getScheduledTask(
                            IntegrityDataGenerationJob.JOB_NAME,
                            IntegrityDataGenerationJob.JOB_GROUP_NAME)
                    .stream()
                    .filter(st -> endpointId.equals(((PublishingEndPoint) st
                            .getProperties()
                            .get(IntegrityUtil.REQUESTER_ENDPOINT))
                            .getId()))
                    .findFirst();
